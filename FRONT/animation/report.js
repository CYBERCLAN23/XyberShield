...
const API_BASE_URL = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1'
  ? 'http://localhost:3001'
  : 'https://xybershield-api.vercel.app'; // Remplacez par votre URL de production Vercel
...
let currentStep = 0; // Déclarer currentStep en dehors de la fonction pour la rendre globale
function initializeForm() {
 ...
 // Afficher la première étape
  ...
  // Fonction pour gérer le clic sur le bouton Suivant (déplacée à l'intérieur de initializeForm)
  ...
  // Fonction pour gérer le clic sur le bouton Précédent (déplacée à l'intérieur de initializeForm)
   ...
   */
  function updateProgressBar() { // Renommée pour éviter la confusion
  ...
  function showStep(step) {
    ...
    console.log('Mise à jour de la barre de progression...');
    updateProgressBar(); // Appel de la fonction renommée
...
// Form submission handler (déplacé à l'intérieur de initializeForm)
if (form) {
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      showUploadProgress(true); // Afficher la barre de progression globale
      ...
      for (let i = 0; i < uploadedFiles.length; i++) {
        ...
        try {
          const response = await new Promise((resolve, reject) => {
            ...
            }; // Correction: point-virgule manquant
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Initialisation du téléchargement de fichiers (appelée après initializeForm pour s'assurer que les éléments sont là)
  initFileUpload();
